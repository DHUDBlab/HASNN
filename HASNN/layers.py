...
class SRGA(nn.Module):
    def __init__(self, in_features,
        ...
        self.snn_1 = neuron.LIF(v_threshold=v_threshold, alpha=alpha, surrogate=surrogate)
        self.snn_2 = neuron.LIF(v_threshold=v_threshold, alpha=alpha, surrogate=surrogate)

        self.dropout = nn.Dropout(dropout)
        self.sizes = sizes

        self.residual_transform = nn.Linear(hids[0], hids[1], bias=False)
        self.norm = nn.LayerNorm(hids[1])
